...
fst_exons_Plot <- dat_melted %>%
  ggplot(aes(x=ordered_var,y=value), axis.line = element_line(colour = 'black', size = 3)) +
  theme_classic() +
  geom_boxplot(lwd=1, fatten=2, outlier.shape=NA, aes(colour="gray"), show.legend = F) +
  ...
  scale_color_manual(values=c("no" = "#4B4B4B", "yes" = "black")) + 
